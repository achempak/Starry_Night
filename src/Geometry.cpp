...
Geometry::Geometry(std::string objFilename, std::string geo_name) : ambient(glm::vec3(1.0f, 0.5f, 0.31f)), diffuse(glm::vec3(1.0f, 0.5f, 0.31f)), specular(glm::vec3(0.5f, 0.5f, 0.5f)), shininess(32.0f), name(geo_name), bounding_on(true)
{
    float width = 640;
    float height = 480;
    glm::vec3 max{0,0,0};
	...
	std::cout << "Geometry VAO: " << vao << std::endl;
    std::cout << "Done initializing object." << std::endl;
}

Geometry::Geometry(std::string objFilename) : Geometry::Geometry(objFilename, ""){}

Geometry::Geometry(std::vector<glm::vec3> pts, std::vector<glm::vec3> norms, std::vector<int> inds, std::string geo_name) : points(pts), normals(norms), indices(inds), ambient(glm::vec3(1.0f, 0.5f, 0.31f)), diffuse(glm::vec3(1.0f, 0.5f, 0.31f)), specular(glm::vec3(0.5f, 0.5f, 0.5f)), shininess(32.0f), name(geo_name), bounding_on(true)
{
	// Set the model matrix to an identity matrix.
	model = glm::mat4(1);
	pos = model;
...
void Geometry::draw(glm::mat4 C, GLuint program)
{
    ...
    GLuint matAmb = glGetUniformLocation(program, "material.ambient");
    GLuint matDiff = glGetUniformLocation(program, "material.diffuse");
    GLuint matSpec = glGetUniformLocation(program, "material.specular");
    GLfloat shine = glGetUniformLocation(program, "material.shininess");
    ...
    glUniform3fv(matAmb, 1, glm::value_ptr(ambient));
    glUniform3fv(matDiff, 1, glm::value_ptr(ambient));
    glUniform3fv(matSpec, 1, glm::value_ptr(ambient));
    ...
    else if(name == "bounding" && bounding_on)
        glDrawElements(GL_LINES, sizeof(int) * indices.size(), GL_UNSIGNED_INT, 0);
    // Unbind from the VAO.
    glBindVertexArray(0);
}

void Geometry::setMaterial(glm::vec3 amb, glm::vec3 diff, glm::vec3 spec, float shine)
{
    ambient = amb;
    diffuse = diff;
    specular = spec;
...
void Geometry::setMaterial(glm::vec3 amb)
{
	ambient = amb;
}

void Geometry::updatePos(glm::mat4 C)
{
    pos = C * model;
}

void Geometry::toggle_bounding(bool on)
{
    bounding_on = on;
}
